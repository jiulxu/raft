...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println("test leader 1 disconnect")
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	fmt.Println("test leader 1 reconnect")
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	fmt.Println("test 2 server disconnected including leader.")
...
func TestRejoin2B(t *testing.T) {
	...
	fmt.Println("TEST: commit 1 called")
	cfg.one(101, servers, true)

	fmt.Println("TEST: leader fails")
	...
	fmt.Println("TEST: new leader fails")
	// new leader network failure
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)

	fmt.Println("TEST: old leader reconnect")
	// old leader connected again
	cfg.connect(leader1)

	cfg.one(104, 2, true)

	fmt.Println("TEST: all together")
	// all together now
	cfg.connect(leader2)
...
func TestBackup2B(t *testing.T) {
	...
	fmt.Println("TEST: TEST 5")
	// lots more commands that won't commit
	for i := 0; i < 30; i++ {
		cfg.rafts[leader2].Start(rand.Int())
	}

	fmt.Println("TEST: TEST 6")
	time.Sleep(RaftElectionTimeout / 2)
...
func TestUnreliableAgree2C(t *testing.T) {
	servers := 5
	fmt.Printf("num of go routine %d \n",runtime.NumGoroutine())
